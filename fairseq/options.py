...
def parse_args_and_arch(parser, input_args=None, parse_known=False):
    ...
    if hasattr(args, 'arch'):
        ...
        ARCH_MODEL_REGISTRY[args.task1_arch].add_args(model_specific_group)
        #ARCH_MODEL_REGISTRY[args.task2_arch].add_args(model_specific_group)

    # Add *-specific args to parser.
    if hasattr(args, 'criterion'):
        CRITERION_REGISTRY[args.criterion].add_args(parser)
    elif hasattr(args, 'task1_criterion'):
        CRITERION_REGISTRY[args.task1_criterion].add_args(parser)
     #   CRITERION_REGISTRY[args.task2_criterion].add_args(parser)
     #   CRITERION_REGISTRY[args.disc_criterion].add_args(parser)

    if hasattr(args, 'optimizer'):
        OPTIMIZER_REGISTRY[args.optimizer].add_args(parser)
    elif hasattr(args, 'task1_optimizer'):
        OPTIMIZER_REGISTRY[args.task1_optimizer].add_args(parser)
     #   OPTIMIZER_REGISTRY[args.task2_optimizer].add_args(parser)
     #   OPTIMIZER_REGISTRY[args.disc_optimizer].add_args(parser)

    if hasattr(args, 'lr_scheduler'):
        LR_SCHEDULER_REGISTRY[args.lr_scheduler].add_args(parser)
    elif hasattr(args, 'task1_lr_scheduler'):
        LR_SCHEDULER_REGISTRY[args.task1_lr_scheduler].add_args(parser)
    #    LR_SCHEDULER_REGISTRY[args.task2_lr_scheduler].add_args(parser)
    #    LR_SCHEDULER_REGISTRY[args.disc_lr_scheduler].add_args(parser)
    ...
    if hasattr(args, 'task1_arch'):
        ARCH_CONFIG_REGISTRY[args.task1_arch](args)
...
def add_multimodal_optimization_args(parser):
    group = parser.add_argument_group('Optimization')
    group.add_argument('--task1-max-epoch', '--me1', default=0, type=int, metavar='N',
                       help='force stop training at specified epoch')
    group.add_argument('--task1-max-update', '--mu1', default=0, type=int, metavar='N',
                       help='force stop training at specified update')
    group.add_argument('--task1-clip-norm', default=25, type=float, metavar='NORM',
                       help='clip threshold of gradients')
    group.add_argument('--task1-sentence-avg', action='store_true',
                       help='normalize gradients by the number of sentences in a batch'
                            ' (default is to normalize by number of tokens)')
    group.add_argument('--task1-update-freq', default='1', metavar='N',
                       help='update parameters every N_i batches, when in epoch i')

    # Optimizer definitions can be found under fairseq/optim/
    group.add_argument('--task1-optimizer', default='nag', metavar='OPT',
                       choices=OPTIMIZER_REGISTRY.keys(),
                       help='Optimizer')
    group.add_argument('--task1-lr', '--learning-rate1', default='0.25', metavar='LR_1,LR_2,...,LR_N',
                       help='learning rate for the first N epochs; all epochs >N using LR_N'
                            ' (note: this may be interpreted differently depending on --lr-scheduler)')
    group.add_argument('--task1-momentum', default=0.99, type=float, metavar='M',
                       help='momentum factor')
    group.add_argument('--task1-weight-decay', '--wd1', default=0.0, type=float, metavar='WD',
                       help='weight decay')

    # Learning rate schedulers can be found under fairseq/optim/lr_scheduler/
    group.add_argument('--task1-lr-scheduler', default='reduce_lr_on_plateau',
                       choices=LR_SCHEDULER_REGISTRY.keys(),
                       help='Learning Rate Scheduler')
    group.add_argument('--task1-lr-shrink', default=0.1, type=float, metavar='LS',
                       help='learning rate shrink factor for annealing, lr_new = (lr * lr_shrink)')
    group.add_argument('--task1-min-lr', default=1e-5, type=float, metavar='LR',
                       help='minimum learning rate')
    group.add_argument('--task1-min-loss-scale', default=1e-4, type=float, metavar='D',
                       help='minimum loss scale (for FP16 training)')
    ###################################################################################
    # Second Task
    ###################################################################################
    group.add_argument('--task2-max-epoch', '--me2', default=0, type=int, metavar='N',
                       help='force stop training at specified epoch')
    group.add_argument('--task2-max-update', '--mu2', default=0, type=int, metavar='N',
                       help='force stop training at specified update')
    group.add_argument('--task2-clip-norm', default=25, type=float, metavar='NORM',
                       help='clip threshold of gradients')
    group.add_argument('--task2-sentence-avg', action='store_true',
                       help='normalize gradients by the number of sentences in a batch'
                            ' (default is to normalize by number of tokens)')
    group.add_argument('--task2-update-freq', default='1', metavar='N',
                       help='update parameters every N_i batches, when in epoch i')

    # Optimizer definitions can be found under fairseq/optim/
    group.add_argument('--task2-optimizer', default='nag', metavar='OPT',
                       choices=OPTIMIZER_REGISTRY.keys(),
                       help='Optimizer')
    group.add_argument('--task2-lr', '--learning-rate2', default='0.25', metavar='LR_1,LR_2,...,LR_N',
                       help='learning rate for the first N epochs; all epochs >N using LR_N'
                            ' (note: this may be interpreted differently depending on --lr-scheduler)')
    group.add_argument('--task2-momentum', default=0.99, type=float, metavar='M',
                       help='momentum factor')
    group.add_argument('--task2-weight-decay', '--wd2', default=0.0, type=float, metavar='WD',
                       help='weight decay')

    # Learning rate schedulers can be found under fairseq/optim/lr_scheduler/
    group.add_argument('--task2-lr-scheduler', default='reduce_lr_on_plateau',
                       choices=LR_SCHEDULER_REGISTRY.keys(),
                       help='Learning Rate Scheduler')
    group.add_argument('--task2-lr-shrink', default=0.1, type=float, metavar='LS',
                       help='learning rate shrink factor for annealing, lr_new = (lr * lr_shrink)')
    group.add_argument('--task2-min-lr', default=1e-5, type=float, metavar='LR',
                       help='minimum learning rate')
    group.add_argument('--task2-min-loss-scale', default=1e-4, type=float, metavar='D',
                       help='minimum loss scale (for FP16 training)')
...
def add_checkpoint_args(parser):
    ...
    group.add_argument('--freeze-encoder', default=False, action='store_true',
                       help='Freeze encoder weights before training')
    return group
...
def add_merging_args(parser):
    group = parser.add_argument_group('Merge models')
    group.add_argument('--encoder', required=True,
                       help="Path to the checkpoint containing the encoder")
    group.add_argument('--decoder', required=True,
                       help="Path to the checkpoint containing the decoder")
    group.add_argument('--new-model-path', required=True,
                       help="Path where to save the new checkpoint")
...
def add_multimodal_model_args(parser):
    group = parser.add_argument_group('Model configuration')
    ...
    group.add_argument(
        ...
        choices=CRITERION_REGISTRY.keys(),
        help='Training Criterion for task 1',
    )
    group.add_argument(
        '--task2-criterion', default='cross_entropy', metavar='CRIT2',
        choices=CRITERION_REGISTRY.keys(),
        help='Training Criterion for task 1',
    )
    group.add_argument(
        '--disc-criterion', default='cross_entropy', metavar='CRIT3',
        choices=CRITERION_REGISTRY.keys(),
        help='Training Criterion for discriminator',
    )
