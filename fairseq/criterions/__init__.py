...
def build_criterion(args, task, model=None):
    if args.task == 'multimodal_translation':
        _criterion = ModuleDict({})
        for model_name in model.models.keys():
            if model_name == 'en-de' or model_name == 'en-fr':
                _criterion[model_name] = CRITERION_REGISTRY[args.task1_criterion](args, task)
            elif model_name == 'h5-en' or model_name == 'npz-en':
                _criterion[model_name] = CRITERION_REGISTRY[args.task2_criterion](args, task)
            else:
                _criterion[model_name] = CRITERION_REGISTRY[args.disc_criterion](args, task)
        return _criterion
    else:
