...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, left_pad=True, convolutions=((512, 3),) * 20, stride=2,
                 audio_features=40, embed_tokens=None):
        super().__init__(dictionary)
        self.dropout = args.dropout
        self.max_source_positions = args.max_source_positions

        self.audio_padding_idx = dictionary.pad()
        ...
        for i, (out_channels, kernel_size, kernel_width) in enumerate(convolutions):
            ...
            self.convolutions.append(
                        Conv2D(in_channels, out_channels, kernel_size,
                            dropout=self.dropout, padding=padding, stride=(1, 2))
            )
            in_channels = out_channels
        self.relu = nn.ReLU()
        if not args.no_attn_2d:
        ...
        self.audio_embed_positions = PositionalEmbeddingAudio(
            args.max_source_positions, args.encoder_embed_dim, self.audio_padding_idx,
            left_pad=left_pad,
        ...
        if embed_tokens is not None:
            ...
            self.text_padding_idx = embed_tokens.padding_idx

            self.embed_tokens = embed_tokens
            self.embed_scale = math.sqrt(embed_dim)
            self.text_embed_positions = PositionalEmbedding(
                args.max_source_positions, embed_dim, self.text_padding_idx,
                left_pad=left_pad,
                learned=args.encoder_learned_pos,
            ) if not args.no_token_positional_embeddings else None
    ...
    def forward(self, src_tokens, src_lengths, lang_pair):
        # For audio input
        if lang_pair.startswith('h5'):
            ...
            x = x + self.audio_embed_positions(x.transpose(0, 1), src_lengths).transpose(0, 1)
        ...
        # For text input:
            ...
            if self.text_embed_positions is not None:
                x += self.text_embed_positions(src_tokens)
            x = F.dropout(x, p=self.dropout, training=self.training)
